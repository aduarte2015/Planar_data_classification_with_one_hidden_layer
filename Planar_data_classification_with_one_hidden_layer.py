...
shape_X = X.shape  # Get the shape of X
shape_Y = Y.shape  # Get the shape of Y
m = X.shape[1]  
...
def layer_sizes(X, Y):
    ...
    n_h = 4           # Hidden layer size (hardcoded to 4 for this exercise)
    n_y = Y.shape[0]  # Number of labels (output size)
...
def forward_propagation(X, parameters):
    ...
    # Ensure A2 has the correct shape (1, m)
...
def compute_cost(A2, Y):
    ...
    cost = -np.sum(logprobs) / m  # average over all examples
...
def backward_propagation(parameters, cache, X, Y):
    ...
    # Retrieve W1, W2 from the dictionary "parameters".
    W1 = parameters["W1"]
    W2 = parameters["W2"]
    
    # Retrieve A1, A2 from the dictionary "cache".
    A1 = cache["A1"]
    A2 = cache["A2"]
    ...
    # dZ1 = W2.T * dZ2 * (1 - A1^2)  (derivative of tanh activation function)
...
def nn_model(X, Y, n_h, num_iterations = 10000, print_cost=False):
    """
    Implements a 2-layer neural network for classification.
    ...
    Returns:
    parameters -- learned parameters from the model
    """
    
    np.random.seed(3)
    
    # Layer sizes
    n_x = X.shape[0]  # Number of input features (in this case 2)
    n_y = Y.shape[0]  # Output size (should be 1)
...
def predict(parameters, X):
    ...
    # Compute the probabilities (A2) using forward propagation
    A2, cache = forward_propagation(X, parameters)
    
    # Classify predictions: if A2 >= 0.5, predict 1 (blue), else 0 (red)
